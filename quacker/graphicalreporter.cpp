...
void GraphicalReporter::reportPosition(const Quackle::GamePosition &position, Quackle::ComputerPlayer *computerPlayer)
{
...

	{
		...
		{
			title = GraphicalBoard::tr("<h2>%1: Turn %2</h2>")
						.arg(QuackleIO::Util::uvStringToQString(position.currentPlayer().name()))
						.arg(position.turnNumber());
		}
		else
		{
		...
		if (m_generateImages)
		{
			...
			const QString filebasename = QString("%1-%2-position.png")
											 .arg(position.turnNumber())
											 .arg(QuackleIO::Util::uvStringToQString(position.currentPlayer().name()));
			...
			else
			{
				QMessageBox::critical(
					0, GraphicalBoard::tr("Error Writing File - Quacker"), GraphicalBoard::tr("Could not write image %1.").arg(filename));
			}
		...
		else
		{
			...
			m_indexStream << QuackleIO::Util::sanitizeUserVisibleLetterString(
				QuackleIO::Util::uvStringToQString(position.board().htmlBoard(boardTileSize)))
						  << m_endl;
		}
	}
	...
	for (Quackle::PlayerList::const_iterator it = players.begin(); it != players.end(); ++it)
	{
		...
		m_indexStream << "<td>" << QuackleIO::Util::uvStringToQString((*it).name()) << "</td>"
					  << "<td>"
					  << QuackleIO::Util::sanitizeUserVisibleLetterString(QuackleIO::Util::uvStringToQString((*it).rack().toString()))
					  << "</td>"
					  << "<td>" << (*it).score() << "</td>"
					  << "</tr>" << m_endl;
	}
		...
		for (Quackle::MoveList::const_iterator it = moves.begin(); it != moves.end(); ++it)
		{
			...
			case Quackle::Move::PlaceError:
			{
				if (m_generateImages)
				{
					...
					const QString filebasename = QString("%1-%2-%3-%4.png")
													 .arg(position.turnNumber())
													 .arg(QuackleIO::Util::uvStringToQString(position.currentPlayer().name()))
													 .arg(QuackleIO::Util::letterStringToQString((*it).prettyTiles()))
													 .arg(QuackleIO::Util::uvStringToQString((*it).positionString()));
					const QString filename = makeFilename(filebasename);
					...
					{
						QMessageBox::critical(0, GraphicalBoard::tr("Error Writing File - Quacker"),
							GraphicalBoard::tr("Could not write image %1.").arg(filename));
					}
				}
				else
				{
				...
				item = item.arg(QuackleIO::Util::sanitizeUserVisibleLetterString(QuackleIO::Util::moveToDetailedString(*it)))
						   .arg((*it).score);
				break;
			}
...
void GraphicalReporter::openIndex()
{
	if (!m_indexStream.device())
	{
		...
		{
			QMessageBox::critical(0, GraphicalBoard::tr("Error Writing File - Quacker"),
				GraphicalBoard::tr("Could not open %1 for writing.").arg(m_indexFile.fileName()));
			return;
		}
