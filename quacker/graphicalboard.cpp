...
void GraphicalBoardFrame::keyPressEvent(QKeyEvent *event)
{
	...
	case Qt::Key_Return:
	case Qt::Key_Enter:
		mode = event->modifiers() & Qt::ShiftModifier || event->modifiers() & Qt::ControlModifier || event->modifiers() & Qt::MetaModifier
			? Commit
			: Submit;
		break;
	...
	default:
		...
		case Append:
			if (event->text().isEmpty() || (event->text().at(0) >= QChar(0xa8) && event->text().at(0) <= QChar(0xb8))
				|| // combining character
...
void GraphicalBoardFrame::prettifyAndSetLocalCandidate(const Quackle::Move &candidate)
{
	...
	if (m_candidate.wordTilesWithNoPlayThru().length() == 1)
	{
		...
		emit statusMessage(tr("Press Enter to add %1 to candidate list, or Control+Enter to commit to it immediately.")
				.arg(QuackleIO::Util::moveToDetailedString(flip(m_candidate))));
	}
	else
	{
		...
		emit statusMessage(tr("Press Enter to add %1 to candidate list, or Control+Enter to commit to it immediately.")
				.arg(QuackleIO::Util::moveToDetailedString(m_candidate)));
	}
}
...
void GraphicalBoardFrame::backspaceHandler()
{
	...
	if (tiles.empty())
	{
		...
		while (true)
		{
			...
			{
				Quackle::Board::TileInformation previousTileInformation(
					m_board.tileInformation(previousTile.height(), previousTile.width()));
...
{
	return (location.width() >= 0 && location.width() < m_boardSize.width() && location.height() >= 0
		&& location.height() < m_boardSize.height());
}
...
QString TileWidget::letterText()
{
	...
	if (metrics.inFont(preferredGlyphs[0]) && metrics.inFont(preferredGlyphs[1]) && metrics.inFont(preferredGlyphs[2]))
		return QString(preferredGlyphs[glyphIndex]);
	else if (metrics.inFont(secondaryGlyphs[0]) && metrics.inFont(secondaryGlyphs[1]) && metrics.inFont(secondaryGlyphs[2]))
...
QPixmap TileWidget::generateTilePixmap()
{
	...
	else
	{
		...
		qDrawShadePanel(
			&painter, borderRect.x(), borderRect.y(), borderRect.width(), borderRect.height(), customPalette, false, borderWidth);
...
{
	return (firstSize.height() + (QUACKLE_MAXIMUM_BOARD_SIZE + 1) * firstSize.width())
		< (secondSize.height() + (QUACKLE_MAXIMUM_BOARD_SIZE + 1) * secondSize.width());
}
